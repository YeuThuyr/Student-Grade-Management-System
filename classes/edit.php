...
if ($classId <= 0) {
    header('Location: ' . BASE_PATH . 'classes/list.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $checkStmt->execute([$values['class_code'], $classId]);
        if ($checkStmt->fetchColumn() > 0) {
            $errors[] = 'Mã lớp đã được sử dụng.';
        }
    }
...
<div class="container py-5 mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold">Chỉnh sửa lớp</h2>
            <p class="text-muted">Cập nhật thông tin lớp học.</p>
        </div>
        ...
        <form method="POST">
            <div class="row g-3">
                ...
                <div class="col-12 text-end">
                    <button type="submit" class="btn btn-hust">Lưu thay đổi</button>
                </div>
            </div>
        </form>
    </div>
