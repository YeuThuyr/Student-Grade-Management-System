...
while ($row = $y_res->fetch_assoc()) $years[] = $row['academic_year'];
...
if ($search_performed) {
    ...
    $having_clause = "";
    if ($f_gpa === 'excellent') $having_clause = " HAVING gpa >= 8.0";
    elseif ($f_gpa === 'good') $having_clause = " HAVING gpa >= 6.5 AND gpa < 8.0";
    elseif ($f_gpa === 'average') $having_clause = " HAVING gpa >= 5.0 AND gpa < 6.5";
    elseif ($f_gpa === 'weak') $having_clause = " HAVING gpa < 5.0";
...
<div class="container py-5 mt-4">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-4 mb-5 bg-white search-section">
        <h5 class="fw-bold mb-3 text-dark"><i class="fas fa-search me-2 text-danger"></i>Tìm kiếm Sinh viên theo Mã số</h5>
        <form method="GET" action="dashboard.php" class="search-form" id="searchForm">
            ...
            <div class="row g-3 mt-2 user-select-none">
                <div class="col-12 col-md-4">
                    ...
                    <select name="academic_year" class="form-select filter-select" onchange="document.getElementById('searchForm').submit()">
                        <option value="">Tất cả các năm</option>
                        <?php foreach($years as $year): ?>
                            <option value="<?php echo htmlspecialchars($year); ?>" <?php if($f_year === $year) echo 'selected'; ?>>Năm học <?php echo htmlspecialchars($year); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-12 col-md-4">
                    <label class="form-label text-muted small fw-bold mb-1">Giới tính</label>
                    <select name="gender" class="form-select filter-select" onchange="document.getElementById('searchForm').submit()">
                        <option value="">Tất cả</option>
                        <option value="Male" <?php if($f_gender === 'Male') echo 'selected'; ?>>Nam</option>
                        <option value="Female" <?php if($f_gender === 'Female') echo 'selected'; ?>>Nữ</option>
                    </select>
                </div>
                <div class="col-12 col-md-4">
                    <label class="form-label text-muted small fw-bold mb-1">Xếp loại GPA</label>
                    <select name="gpa_range" class="form-select filter-select" onchange="document.getElementById('searchForm').submit()">
                        <option value="">Tất cả mức điểm</option>
                        <option value="excellent" <?php if($f_gpa === 'excellent') echo 'selected'; ?>>Xuất sắc (≥ 8.0)</option>
                        <option value="good" <?php if($f_gpa === 'good') echo 'selected'; ?>>Khá (6.5 - 7.9)</option>
                        <option value="average" <?php if($f_gpa === 'average') echo 'selected'; ?>>Trung bình (5.0 - 6.4)</option>
                        <option value="weak" <?php if($f_gpa === 'weak') echo 'selected'; ?>>Yếu (< 5.0)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="search-hint mt-3">
                ...
                Có thể kết hợp tìm kiếm mã số và các bộ lọc. Biểu đồ và thẻ tóm tắt cũng sẽ tự động đồng bộ theo bộ lọc này.
        </form>
        ...
        <div class="search-results mt-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h6 class="fw-bold text-dark mb-0">
                    Kết quả tìm kiếm cho "<span class="text-danger"><?php echo htmlspecialchars($search_code); ?></span>"
                </h6>
                ...
                <table class="table search-table align-middle" id="searchResultsTable">
                    ...
                    <tbody>
                        ...
                        <tr class="search-result-row" style="animation-delay: <?php echo $index * 0.05; ?>s;">
                            ...
                            <td>
                                <span class="student-code-badge"><?php echo htmlspecialchars($student['student_code']); ?></span>
                            </td>
                            ...
                            <td>
                                <?php if ($student['gpa'] !== null): ?>
                                    ...
                                    $gpa_class = 'gpa-low';
                                    if ($gpa >= 8.0) $gpa_class = 'gpa-excellent';
                                    elseif ($gpa >= 6.5) $gpa_class = 'gpa-good';
                                    elseif ($gpa >= 5.0) $gpa_class = 'gpa-average';
                                    ?>
            ...
            <div class="no-results text-center py-5">
                ...
                <p class="text-muted mb-0">Không có sinh viên nào có mã bắt đầu bằng "<strong><?php echo htmlspecialchars($search_code); ?></strong>".<br>Hãy thử nhập lại mã sinh viên khác.</p>
            </div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
...
.search-table thead th:first-child { border-radius: 0.5rem 0 0 0.5rem; }
.search-table thead th:last-child { border-radius: 0 0.5rem 0.5rem 0; }
...
.search-table tbody tr td:first-child { border-radius: 0.5rem 0 0 0.5rem; }
.search-table tbody tr td:last-child { border-radius: 0 0.5rem 0.5rem 0; }
