...
$values = ['subject_code' => '', 'subject_name' => '', 'credit' => '', 'description' => ''];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt->execute([$values['subject_code']]);
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare('INSERT INTO subjects (subject_code, subject_name, credit, description) VALUES (?, ?, ?, ?)');
        $stmt->execute([$values['subject_code'], $values['subject_name'], $values['credit'], $values['description']]);
        header('Location: ' . BASE_PATH . 'subjects/list.php?success=1');
        exit();
    }
}
...
<div class="container py-5 mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted">Tạo môn học để gắn vào bảng điểm.</p>
        </div>
        ...
        <form method="POST">
            <div class="row g-3">
                ...
                <div class="col-12 text-end">
                    <button type="submit" class="btn btn-hust">Lưu môn học</button>
                </div>
            </div>
        </form>
    </div>
